...
RSpec.describe PeriodChain do
  context '#valid?' do
    ...
    example 'Example 5' do
      period_chain = PeriodChain.new('04.06.1976', ["1976M6D4", "1976M6D5", "1976M6D6"])
      expect(period_chain.valid?).to eq(true)
    end

    example 'Example 6' do
      period_chain = PeriodChain.new('02.05.2023', ["2023M5D2", "2023M5D3", "2023M5D5"])
      expect(period_chain.valid?).to eq(false)
    end

		example 'Example 7' do
      period_chain = PeriodChain.new('30.01.2023', ["2023M1", "2023M2", "2023M3D30"])
      expect(period_chain.valid?).to eq(true)
    end

		example 'Example 8' do
			period_chain = PeriodChain.new('31.01.2023', ["2023M1", "2023M2", "2023M3D30"])
			expect(period_chain.valid?).to eq(false)
		end

		example 'Example 9' do
      period_chain = PeriodChain.new('30.01.2020', ["2020M1", "2020", "2021", "2022", "2023", "2024M2", "2024M3D30"])
      expect(period_chain.valid?).to eq(true)
    end

		example 'Example 10' do
			period_chain = PeriodChain.new('30.01.2020', ["2020M1", "2020", "2021", "2022", "2023", "2024M2", "2024M3D29"])
			expect(period_chain.valid?).to eq(false)
		end
